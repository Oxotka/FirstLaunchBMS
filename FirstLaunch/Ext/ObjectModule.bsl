...
Procedure ЗагрузитьТаблицуПредопределенных(ЧтениеXML)
	...
	ВременнаяСтрока = TableOfPredifined.Add();
...
Procedure ИнициализироватьСериализаторXDTOСАннотациейТипов()
	...
	ТипыСАннотациейСсылок = PredifinedTypeForUnload();
...
Function ОбъектОбразуетСсылочныйТип(ObjectMD)
	
	If ObjectMD = Undefined Then
		Return False;
	EndIf;
	
	If Метаданные.Справочники.Содержит(ObjectMD)
		ИЛИ Метаданные.Документы.Содержит(ObjectMD)
		ИЛИ Метаданные.ПланыВидовХарактеристик.Содержит(ObjectMD)
		ИЛИ Метаданные.ПланыСчетов.Содержит(ObjectMD)
		ИЛИ Метаданные.ПланыВидовРасчета.Содержит(ObjectMD)
		ИЛИ Метаданные.ПланыОбмена.Содержит(ObjectMD)
		ИЛИ Метаданные.БизнесПроцессы.Содержит(ObjectMD)
		ИЛИ Метаданные.Задачи.Содержит(ObjectMD) Then
		Return True;
	EndIf;
...
Procedure ЗаменитьСсылкиНаПредопределенные(FileName)
	
	ПотокЧтения = New ЧтениеТекста(FileName);
	
	TempFile = ПолучитьИмяВременногоФайла("xml");
	
	ПотокЗаписи = New ЗаписьТекста(TempFile);
